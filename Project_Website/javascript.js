...
// Configuration
const API_BASE_URL = "https://symmetrical-space-fortnight-r464p9x54g56fp7qq-8500.app.github.dev/";

// Initialize the application
function init() {
    $.ajaxSetup({async: false});
    let link = API_BASE_URL;
...
function filterByGenre(genreName, buttonElement) {
    let filteredGames;
    ...
    updateFilterButtons('genre', genreName, buttonElement);
}

// Filter games by platform (client-side filtering)
function filterByPlatform(platformName, buttonElement) {
    let filteredGames;
    ...
    generateCards(filteredGames);
    updateFilterButtons('platform', platformName, buttonElement);
}
...
function updateFilterButtons(filterType, activeFilter, buttonElement) {
    ...
    // Fallback if the calling button was not provided
